...
login_type = False
file_path = open('path_file', 'r').read()
...
def create_depart_folder(create_table_location, target_line):
    ...
    conn.commit()
    conn.close()
    os.makedirs(file_path + target_line, exist_ok=True)
...
def create_line_folder(create_table_location, target_line):
    ...
    for station in stations:
        ...
        os.makedirs(file_path + create_table_location + '/' + target_line + '/' + path_suffix + station, exist_ok=True)
        ...
        if create_value:
            ...
            if check_value:
                ...
                if checkbox_value == 'true':
                    ...
                    os.makedirs(file_path + create_station_location + '/' + path_suffix + '/' + create_value,
                                exist_ok=True)
            conn.connection.commit()
            conn.close()
        else:
